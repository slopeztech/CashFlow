...
def run_platform_update(*, initiated_by='manual'):
    ...
    with open(log_path, 'w', encoding='utf-8') as log_file:
        ...
        steps = [
            ('Fetch remote changes', ['git', 'fetch', '--all', '--prune']),
        ...
        if os_name == 'linux':
            linux_steps = [
                ...
                ('Restart CashFlow service', ['systemctl', 'restart', UPDATE_SERVICE_NAME]),
                ('Verify CashFlow service status', ['systemctl', 'is-active', UPDATE_SERVICE_NAME]),
            ]
